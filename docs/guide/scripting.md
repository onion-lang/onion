...
space (`prefix "..."`) it is an ordinary identifier followed by a string. A
reserved keyword immediately followed by a string is unaffected — `return"x"`
stays `return "x"`, not a scheme call. An undefined prefix is a normal "method
not found" error, not a lexer error.
