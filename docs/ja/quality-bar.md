...
| 2 | サンプルの健全性 | `SampleCompilesSpec` / `SampleProgramsSpec`（どちらも `run/*.on` 全件をコンパイル） | 75 / 75 compile | すべてコンパイル、rot なし |
| 3 | 大規模プログラム | 100行以上の `run/*.on` をそのまま end-to-end で実行できる数 | 18（Blackjack、BrokenLogDemo、BudgetTracker、GradeBook、GraphSearch、Inventory、LibraryCatalog、LibrarySystem、MiniRpg、OrderReport、PayrollReport、PlaylistManager、ShapeProcessor、StatsApp、TextAnalyzer、TodoManager、ToolDemo、TournamentTracker） | ≥ 5 |
