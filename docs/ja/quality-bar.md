...
|---|-----------|----------------|----------------------|----------------|
| 1 | テストスイート | `sbt shutdown && sbt -Duser.language=en testFull`（下記の注記を参照） | 3628 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
| 2 | サンプルの健全性 | `SampleCompilesSpec` / `SampleProgramsSpec`（どちらも `run/*.on` 全件をコンパイル） | 170 / 170 compile | すべてコンパイル、rot なし |
