...
| 2 | サンプルの健全性 | `SampleCompilesSpec` / `SampleProgramsSpec`(どちらも `run/*.on` 全件をコンパイル) | 410 / 410 compile | すべてコンパイル、rot なし |
| 3 | 大規模プログラム | 100行以上の `run/*.on` をそのまま end-to-end で実行できる数 | 343(全件のアルファベット順一覧は `run/` を参照——ここでは列挙しない。下記の注記を参照) | ≥ 5 |
