...
ベースライン数値は 2026-07-26 時点（develop @ 871fe5a1）の実測値です。前回のベースライン
（2026-06-26 @ 6a2d0e4）は持ち越さず**測り直しました**。乖離が誤解を招く水準に達していた
ためです——テスト数は 1193 と記録されていましたが実際は 2445、サンプルは 36 に対し 59、
診断コードは ~24 に対し 74 でした。

| # | 次元 | 測定方法 | 現在値（2026-07-26） | 合格閾値 |
|---|-----------|----------------|----------------------|----------------|
| 1 | テストスイート | `sbt -batch -Duser.language=en test` | 2590 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
