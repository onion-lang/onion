...
|---|-----------|----------------|----------------------|----------------|
| 1 | テストスイート | `sbt -batch -Duser.language=en test` | 2979 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
| 2 | サンプルの健全性 | `SampleCompilesSpec` / `SampleProgramsSpec`（どちらも `run/*.on` 全件をコンパイル） | 65 / 65 compile | すべてコンパイル、rot なし |
| 3 | 大規模プログラム | 100行以上の `run/*.on` をそのまま end-to-end で実行できる数 | 8（BrokenLogDemo、GradeBook、OrderReport、ShapeProcessor、StatsApp、TextAnalyzer、TodoManager、ToolDemo） | ≥ 5 |
