...
- **代入・宣言** — 非 null 型が期待される箇所に nullable な値を代入した場合: `val`/`var` 宣言の初期化子、`return` の値、単純な再代入、フィールドの初期化子（代入先自体が null を許容する場合 -- nullable 型で宣言されている、または bare/platform な型パラメータである場合 -- は対象外）。
- **`catch` 節の型** — `catch e: E?` のように、宣言された例外の型が nullable な場合。上記の各形式と異なり、ここには null チェックすべき値そのものが存在しない -- catch される値が null になることはないため、対処は catch 節の宣言型から `?` を取り除くこと。
...
`E` コードと異なり `W` コードは既定ではコンパイルを失敗させません（`--warn error` で
エラー扱いにできます）。`--Wno` で個別に抑制することもできます（各コードの `--Wno` 名は
[コンパイラツール](../tools/compiler.md#-wno-codes) を参照）。

**(予約)** と付いたコードは宣言されており `--Wno` で名指しできますが、実際に報告する
コンパイラのパスがまだ無いため、ビルドログに出現することはありません。

| コード | 意味 |
|--------|------|
| `W0001` | 未使用の変数 |
| `W0002` | 未使用の import（予約） |
| `W0003` | 到達不能コード |
| `W0004` | 非推奨の機能（予約） |
| `W0005` | 変数のシャドーイング |
| `W0006` | 未使用の引数 |
| `W0007` | 空のブロック（予約） |
| `W0008` | 冗長なキャスト（予約） |
| `W0009` | null 参照の可能性（予約） |
| `W0010` | 不要な型変換（予約） |
| `W0011` | 未検査キャスト（予約） |
| `W0012` | 非 null 型への null 代入 |
| `W0013` | 疑わしい文字列補間構文 |
...
上の各節は遭遇しやすいコードを例つきで説明しています。この表は `E` と `I` の全コードの
一覧なので、ビルドログで見たコードは必ずここで引けます。`W` コードは上の[警告](#警告)を
参照してください。
