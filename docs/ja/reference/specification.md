...
12. ビット OR: `|`
13. 論理 AND: `&&`
14. 論理 OR / Elvis: `||` `?:`（同じ優先順位、左から右へ）
15. 代入: `=` および複合代入 `+=` `-=` `*=` `/=` `%=` `&=` `|=` `^=` `<<=` `>>=` `>>>=`

各行はその下の行より強く結合します。`6 | 1 & 2` は `(6 | 1) & 2` ではなく
`6 | (1 & 2)`、`a || b && c` は `(a || b) && c` ではなく `a || (b && c)` です。
...
上記のスカラー限定の形に加えて、次の2つのパラメータ形も受け付けられます:

```onion
def main(args: String[]): void { ... }              // 生の argv（未パース）
def main(cmd: String, files: String[]): void { ... } // スカラーの接頭列 + rest コレクタ
```

単一の `String[]` パラメータは、パースされていない生のコマンドライン引数を
受け取ります（従来の Java 流のエントリーポイント）。それ以外がスカラーである
パラメータ列の *末尾* にある `String[]` パラメータは、代わりに必須の先頭パラメータを
消費したあとの残りの引数すべてを集めます。それ以外の位置にある `String[]`
パラメータ — 後ろにさらにパラメータが続く先頭や、途中にあるもの — はコンパイル
エラーです。唯一のパラメータ（生の argv）か、最後のパラメータ（rest コレクタ）の
どちらかでなければなりません。
...
1. ジェネリクスは消去ベース（型引数は不変 — 変性・ワイルドカード・reified 型情報は
   なし）。ただし、型パラメータの null 性はコンパイル時に追跡される
2. 末尾呼び出し最適化は直接自己再帰と相互自己再帰をカバー。一般的な CPS は対象外
3. 一部の診断メッセージは理想的にはパイプラインのより早い段階で報告されるべき

コンパイラは、変異ファザー・クラッシュ再現コーパス・コード生成正当性テストによって
「クラッシュしない／誤コンパイルしない」基準を維持しています。もしクラッシュや誤
コンパイルに遭遇したら、最小の再現例を報告してください。
