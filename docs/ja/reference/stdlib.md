...
`describe()` は診断用にその shape が何を読むのかを名付けます——正規表現ベースの shape
なら `re"\d+-\d+"` のように。`print`/`printLossless` が投げる `UnsupportedOperationException`
はこれを引用して「どの」shape が拒否したのかを伝えます。「この shape は可逆ではありません」
だけでは、どれのことか分からないからです。
