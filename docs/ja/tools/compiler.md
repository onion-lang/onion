...
説明に **（予約）** と付いたコードはここで受け付けられますが、実際に検出するパスが
まだ無いため、抑制しても効果はありません。

| コード | 名前 | 説明 |
|------|------|--------------|
| `W0001` | `unused-variable` | 未使用の変数 |
| `W0002` | `unused-import` | 未使用の import（予約） |
| `W0003` | `unreachable-code` | 到達不能コード |
| `W0004` | `deprecated`, `deprecated-feature` | 非推奨の機能（予約） |
| `W0005` | `shadowed-variable` | 変数のシャドーイング |
| `W0006` | `unused-parameter` | 未使用の引数 |
| `W0007` | `empty-block` | 空のブロック（予約） |
| `W0008` | `redundant-cast` | 冗長なキャスト（予約） |
| `W0009` | `possible-null-deref`, `null-deref` | null 参照の可能性（予約） |
| `W0010` | `unnecessary-conversion` | 不要な型変換（予約） |
| `W0011` | `unchecked-cast` | 未検査キャスト（予約） |
