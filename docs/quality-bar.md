...
|---|-----------|----------------|----------------------|----------------|
| 1 | Test suite | `sbt shutdown && sbt -Duser.language=en testFull` (see the note below) | 3792 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 172 / 172 compile | all compile, no rot |
