...
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 320 / 320 compile | all compile, no rot |
| 3 | Large programs | count of `run/*.on` ≥ 100 lines that run end-to-end as-is | 253 (see `run/` for the full, alphabetized list — no longer enumerated here, see note below) | ≥ 5 |
