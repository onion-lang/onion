...
|---|-----------|----------------|----------------------|----------------|
| 1 | Test suite | `sbt shutdown && sbt -Duser.language=en testFull` (see the note below) | 3714 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 171 / 171 compile | all compile, no rot |
