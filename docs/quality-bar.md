...
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 75 / 75 compile | all compile, no rot |
| 3 | Large programs | count of `run/*.on` ≥ 100 lines that run end-to-end as-is | 18 (Blackjack, BrokenLogDemo, BudgetTracker, GradeBook, GraphSearch, Inventory, LibraryCatalog, LibrarySystem, MiniRpg, OrderReport, PayrollReport, PlaylistManager, ShapeProcessor, StatsApp, TextAnalyzer, TodoManager, ToolDemo, TournamentTracker) | ≥ 5 |
