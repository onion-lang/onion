...
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 67 / 67 compile | all compile, no rot |
| 3 | Large programs | count of `run/*.on` ≥ 100 lines that run end-to-end as-is | 10 (Blackjack, BrokenLogDemo, LibrarySystem, MiniRpg, OrderReport, ShapeProcessor, StatsApp, TextAnalyzer, TodoManager, ToolDemo) | ≥ 5 |
