...
|---|-----------|----------------|----------------------|----------------|
| 1 | Test suite | `sbt -batch -Duser.language=en test` | 2979 pass / 0 fail / 1 cancelled | 0 failed, 0 skipped |
| 2 | Sample health | `SampleCompilesSpec` / `SampleProgramsSpec` (both compile every `run/*.on`) | 65 / 65 compile | all compile, no rot |
| 3 | Large programs | count of `run/*.on` ≥ 100 lines that run end-to-end as-is | 8 (BrokenLogDemo, GradeBook, OrderReport, ShapeProcessor, StatsApp, TextAnalyzer, TodoManager, ToolDemo) | ≥ 5 |
