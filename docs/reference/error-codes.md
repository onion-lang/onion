...
is derived from them. Anything else has no argument representation.
...
This used to be silent: the CLI synthesis simply skipped such a script, which then
compiled to a program with no `main` and nothing to run (issue #424).
