...
trait TypedASTVisitor[T]:
  ...
  def visitSafeStaticExtensionCall(node: SafeStaticExtensionCall): T
  ...
  def visitTerm(term: Term): T = term match
    // In frequency order: locals, calls, constants and casts are most of a method's terms, and
    // every case is an instanceof test the term has to fall through.
    ...
    case n: SafeStaticExtensionCall => visitSafeStaticExtensionCall(n)
...
abstract class DefaultTypedASTVisitor[T] extends TypedASTVisitor[T]:
  ...
  override def visitSafeStaticExtensionCall(node: SafeStaticExtensionCall): T = defaultValue
