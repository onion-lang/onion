  ...
  private def runPhase[In, Out](
    ...
    try {
      ...
      case e: CompilationException =>
        ...
        None
      case e @ (_: VerifyError | _: ClassFormatError) =>
        // The compiler generated bytecode that fails JVM verification (surfaced,
        // for example, when a later phase loads a generated class). That is a
        // compiler bug; report it as an internal error (I0000) rather than
        // letting the Error escape and crash with a raw stack trace.
        ctx.timings += PhaseTiming(phase.name, now() - start, countOf(input), 0)
