...
/**
 * The JS/TS-style-`constructor` hint (`SyntaxHintClassifier`'s
 * `JsStyleConstructorDeclaration` case) must resolve through the bilingual
 * `error.parsing.hint.*` bundle in both locales, like every other hint in
 * that match -- see JavaStyleConstructorHintI18nSpec for the Java-style twin.
 */
class JsStyleConstructorHintI18nSpec extends AnyFunSpec with Diagrams {
  ...
  it("fires for a JavaScript/TypeScript-style `constructor(...) { }` method") {
