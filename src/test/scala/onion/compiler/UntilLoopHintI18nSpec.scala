 ...
 */
class UntilLoopHintI18nSpec extends AnyFunSpec with Diagrams {
  private val key = "error.parsing.hint.until_not_supported"
  ...
  it("fires for a Ruby-style `until` loop") {
    ...
    assert(msgs.contains("while !"), s"expected the hint's `while !` example, got: $msgs")
  }
}
